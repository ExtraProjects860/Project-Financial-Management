...
@financial_accounts_routes.route("/create-financial-accounts/<uid>", methods=["POST"])
@UserController.login_required
def create_financial_accounts(uid: str) -> tuple[Response, int]:
    try:
        ...
        return jsonify({"success": "Financial account created successfully"}), 201
    except Exception as e:
        return jsonify({"error": "Error creating financial accounts: " + str(e)}), 500


@financial_accounts_routes.route("/update-financial-accounts/<uid>", methods=["PUT"])
@UserController.login_required
def update_financial_accounts(uid: str) -> tuple[Response, int]:
    try:
        data = request.get_json()
        ...
        return jsonify({"success": "Financial account updated successfully"}), 200
    except Exception as e:
        return jsonify({"error": "Error updating financial accounts: " + str(e)}), 500
    

@financial_accounts_routes.route("/delete-financial-accounts/<uid>", methods=["DELETE"])
@UserController.login_required
def delete_financial_accounts(uid: str) -> tuple[Response, int]:
    try:
        data = request.get_json()
        ...
        return jsonify({"success": "Financial account deleted successfully"}), 200
    except Exception as e:
